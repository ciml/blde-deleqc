...
int VARIANTE;       // variante do doub
double TOLERANCE;    // talerancia para o criterio de parada
		...
		for(int i = 0; i < SIZEL; i++){
			...
			for(int j = 0; j < DIMF; j++){
				if (j == jRand || rand()/(float)RAND_MAX < CR){
                                    ...
                                    if (VARIANTE == 1){         //DE/rand/1/bin
					u[j] = popF[ind1][j] + F*(popF[ind2][j] - popF[ind3][j]);
                                        
                                    } else if (VARIANTE == 2){  //DE/best/1/bin
					u[j] = uF[j] + F*(popF[ind2][j] - popF[ind3][j]);
                                        
                                    } else if (VARIANTE == 3){  //DE/target-to-rand/1/bin
					u[j] = popF[i][j] + F*(popF[ind1][j] - popF[i][j]) + F*(popF[ind2][j] - popF[ind3][j]);
                                        
                                    } else if (VARIANTE == 4){  //DE/target-to-best/1/bin
...
/////////////////////////////////////////////
    for(int i = 0; i < SIZEL; i++){
        ...
        if (iguais(u, popL[i], DIML) && (iguais(uF, popLValoresF[i], DIMF) == 0)){
...
void inicializa(double** &pop, int n, int d, int nivel){
	...
	for (int i = 0; i < n; i++){
		...
		//Gera valores aleatorios para o seguidor
		if (nivel == 1){
//			for(int j = 0; j < DIMF; j++){
//				popLValoresF[i][j] = getLower(nivel, FUNCAO, j) + (rand()/(double)RAND_MAX)*(getUpper(nivel, FUNCAO, j) - getLower(nivel, FUNCAO, j)); //UPPER - LOWER
//			}

			//Busca y com x fixo
//			calculaAptidao(popLValoresF[i], DIMF, 2, pop[i], popLValoresF[i]);
//			double *uF = new double[DIMF + 2];
