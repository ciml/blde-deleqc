...
int compara(double *ind1, double *ind2, int d, int nivel){
    ...
    if (nivel == 1){
    ...
    } else if (ind1[d+1] > EPS && ind2[d + 1] > EPS){ //ambos violam as rest.
        if (ind1[d+1] <= ind2[d + 1]){ // verifica quem viola menos
            return 1;
        } else {
            return 0;
        }        
       ...
       if (ind1[d+2] <= 0 && ind2[d+2] > 0){
            ...
            return 0; //ind2 nao violou
        } else if (ind1[d+2] > 0 && ind2[d + 2] > 0){ //ambos violam as rest.
            if (ind1[d+2] <= ind2[d + 2]){ // verifica quem viola menos
               return 1;
          } else {
               return 0;
        }
...
//        cout << "  " << u[0] << "  " << u[1] << endl;
        ...
        if ( (iguais(u, popL[i], DIML) == 1) && (iguais(uF, popLValoresF[i], DIMF) == 0) ){
            ...
            if (compara(uF, popLValoresF[i], DIMF, 2) > 0){ // uF melhor
...
//    cout << " " << leader[0] << " " << leader[1] << endl;
   ...
   // Calcula o valor de x0
    ...
    i = 0;
    while(i < n){  //Enquanto número de indivíduos total da população
