...
//-----------------------------------------------------------------------------
		...
		break;
                case 25: //P.4 com epsilon (para  blde)
			if (nivel == 1){
				return 2;
			} else if (nivel == 2){
				return 6;
		...
		break;
                case 26: //P.5 com epsilon (para o blde)
			if (nivel == 1){
				return 2;
			} else if (nivel == 2){
				return 6;
...
//---------------------------------------------------------------------------
		...
		break;
                case 25: //P.4 com epsilon (para o blde)
			if (nivel == 1){
				return 1;
			} else if (nivel == 2){
				return 1;
...
//----------------------------------------------------------------------------
		...
		//Savard,94 - (1.889,0.889) f1=-1.407, f2=7.61
		double x, y1, y2;
		double fit = 0, rest = 0;
		if (nivel == 1){
			x = ind[0];
	...
	///// Novos exemplos para a apresentação Calvete2004
        ...
        /////====== Novos exemplos CEC2016 ======
        else if (funcao == 20){ // P.1	(F.3 do cec2013)
		double x1, x2, y1, y2, y3, y4, y5, y6;
		double fit = 0, rest = 0, vb = 0;
		...
		if (nivel == 1){
			...
			fit = (-x1 - 2*x2 - y1 - y2 - 2*y3);
                        ...
                        // Restricoes lineares
			rest = 0.0;
			double v = fabs(-y1 + y2 + y3 + y4 - 1);
			//if (v > EPS){
				rest += v;
			//}
			v = fabs(2*x1 - y1 + 2*y2 - 0.5*y3 + y5 - 1);
			//if (v > EPS){
				rest += v;
			//}
			v = fabs(2*x2 + 2*y1 - y2 - 0.5*y3 + y6 - 1);
			//if (v > EPS){
				rest += v;
			//}
        ...
        else if (funcao == 21){ //P.2	(F.4 do cec2013)	
		double x1,x2, y1, y2, y3, y4, y5;
		double fit = 0, rest = 0, vb = 0;
		...
		if (nivel == 1){
			...
			fit = -x1 -x2 + 4*y1 - y2;
                        ...
                        // Restricoes lineares
			rest = 0.0;
			double v = fabs(-2*x1 + y1 - y2 + y3 + 2.5);
			//if (v > EPS){
				rest += v;
			//}
			v = fabs( x1 - 3*x2 + y2 + y4 - 2);
			//if (v > EPS){
				rest += v;
			//}
			v = fabs( x1 + x2 + y5 - 2);
        ...
        else if (funcao == 22){ // P.3	(F.9 do cec2013)	
		...
		if (nivel == 1){
			...
			rest = 0.0;
			double v = fabs(4*x + 5*y1 + 4*y2 + y3 - 12);
			//if (v > EPS){
				rest += v;
			//}
			v = fabs(-4*x - 5*y1 + 4*y2 + y4 + 4);
			//if (v > EPS){
				rest += v;
			//}
			v = fabs(4*x - 4*y1 + 5*y2 + y5 - 4);
			//if (v > EPS){
				rest += v;
			//}
			v = fabs(-4*x + 4*y1 + 5*y2 + y6 - 4);
			//if (v > EPS){
				rest += v;
			//}
	...
	else if (funcao == 23){ //P.4 (B.6 do artigo do Li)
		double x1, x2, y1, y2, y3, y4, y5, y6;
		double fit = 0, rest = 0, vb = 0;                
                double v;
		...
		if (nivel == 1){
			...
			fit = (1 + x1 + x2 + 2*y1 - y2 + y3)/(6 + 2*x1 + y1 + y2 - 3*y3); 

                        // Restricoes lineares
			rest = 0.0;                        
                        v = fabs( -y1 + y2 + y3 + y4 - 1);                       
			//if ( v > EPS ){
				rest += v;
			//}					
	...
	else if (funcao == 24){ //P.5 (B.10 do artigo do Li)
		double x1, x2, y1, y2, y3, y4, y5, y6;
		double fit = 0, rest = 0, vb = 0;
                double v;
		...
		if (nivel == 1){
			...
			fit = fabs(-8*x1 - 4*x2 + 4*y1 - 40*y2 - 4*y3 + 29.2);
        ...
        else if (funcao == 25){ //P.4 com epsilon ( para o blde )
		double x1, x2, y1, y2, y3, y4, y5, y6;
		double fit = 0, rest = 0;                
                double v;
		...
		if (nivel == 1){
			...
			fit = (1 + x1 + x2 + 2*y1 - y2 + y3)/(6 + 2*x1 + y1 + y2 - 3*y3); 
                        ...
                        // Restricoes lineares de igualdade
			rest = 0.0;                        
                        v = fabs(- y1 + y2 + y3 + y4 - 1);                       
			if ( v > EPS ){
				rest += v;
	...
	else if (funcao == 26){ //P.5 com epsilon ( para o blde )
		double x1, x2, y1, y2, y3, y4, y5, y6;
		double fit = 0, rest = 0;
                double v;
